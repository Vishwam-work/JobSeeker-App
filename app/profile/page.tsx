...
export default function Profile() {
  ...
  useEffect(() => {
    if (profileData.personalInfo.countryId) {
      fetch(
        `http://localhost:8000/master/api/states/?country_id=${profileData.personalInfo.countryId}`
      )
        .then((res) => res.json())
        .then((data) => {
          setStates(data);
  ...
  useEffect(() => {
    if (profileData.personalInfo.stateId) {
      fetch(
        `http://localhost:8000/master/api/cities/?state=${profileData.personalInfo.stateId}`
      )
        .then((res) => res.json())
        .then(setCities)
        .catch((err) => console.error(err));
  ...
  useEffect(() => {
    fetch("http://localhost:8000/master/api/companies/")
      .then((res) => res.json())
      .then((data) => {
        setCompanies(data);
      })
  ...
  useEffect(() => {
    if (experienceForm.category) {
      fetch(
        `http://localhost:8000/master/api/jobs_title/?category=${experienceForm.category}`
      )
        .then((res) => res.json())
        .then((data) => {
          setJobTitles(data);
  ...
  const uploadResume = async () => {
    ...
    const res = await fetch(
      "http://localhost:8000/api/profile/upload-resume/",
      {
        method: "PATCH",
        headers: {
                     ...
                     <Button
                      ...
                      onClick={() => {
                        if (profileData?.personalInfo?.resume) {
                          window.open(`http://localhost:8000${profileData.personalInfo.resume}`, "_blank");
                        } else {
